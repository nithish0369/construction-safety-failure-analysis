...
### Finding 2: Violations Are Visually Harder to Detect
Interestingly, violation classes are **not underrepresented** in the training data:

| Type | Classes | Total Instances |
|---|---|---|
| Compliance | Hardhat + Safety Vest + Mask | 7,851 |
| Violation | NO-Hardhat + NO-Safety Vest + NO-Mask | 9,161 |

This means the failure isn't just a class imbalance problem — **violations are inherently harder to detect visually**. A missing hardhat has no distinct visual feature; the model must infer absence, not presence. This is a stronger and more nuanced finding than simple imbalance.

### Finding 3: Dataset Noise
Manual inspection of failure images revealed out-of-domain images in the dataset — urban streets, indoor scenes, and a boat race — contributing to unreliable detections and highlighting a critical data quality issue.
...
![Sample Training Images](images/sample_images.png)
...
1. **Clean the dataset** — remove out-of-domain images that add noise
2. **Use mAP50-95 as primary metric** for safety-critical systems — stricter overlap threshold better reflects real-world reliability
3. **Weight recall higher** for violation classes — missing a violation is more dangerous than a false alarm
4. **Explore harder negative mining** — since violations are visually harder to detect, not just rarer
5. **Augment violation examples** with varied backgrounds and lighting conditions
...
│   ├── missed_detections_chart.png  # Failure analysis chart
│   └── failure_examples.png     # Failure image grid
